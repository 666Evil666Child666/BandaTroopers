...
/datum/ai_action/select_primary/get_context_weight(datum/human_ai_context/context)
	...
	if(!inventory.has_secondary_weapons())
		return 0

	if(!brain.has_tried_reload() && inventory.has_primary_weapon())
		return 0

	if(controller.can_use_item(inventory.get_primary_weapon()))
		return 0
...
/datum/ai_action/select_primary/proc/decide_primary_weapon()
	...
	var/obj/item/weapon/gun/best_secondary = inventory.get_next_secondary_weapon()
	if(!best_secondary)
		return
	...
	if(primary_weapon && controller.is_holding(primary_weapon))
		var/possible_storage_loc = inventory.storage_has_room(primary_weapon)
		if((primary_weapon.flags_equip_slot & SLOT_BACK) && !controller.get_back())
			...
			inventory.store_item(primary_weapon, possible_storage_loc)

	inventory.add_secondary_weapon(primary_weapon)
	inventory.set_primary_weapon(best_secondary)
