...
/datum/unit_test/halo_preset_coverage/proc/validate_grenade_throwback_rule(preset_path, expected_can_throw_back)
	...
	if(brain.grenade.can_throw_back_grenades != expected_can_throw_back)
		Fail("[preset_path] expected can_throw_back_grenades [expected_can_throw_back], got [brain.grenade.can_throw_back_grenades]", __FILE__, __LINE__)
	qdel(brain)
	qdel(test_human)
	qdel(preset)
